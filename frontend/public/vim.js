(function () {
  ...
  function moveVerticalPos(view, direction, count, posOverride) {
    ...
    if (desiredCol == null) {
      desiredCol = pos - current.start;
    }

    return target.start + Math.min(desiredCol, target.end - target.start);
  }
  ...
  function motionTarget(view, key, count, posOverride) {
    ...
    switch (key) {
      case "h":
      case "ArrowLeft": {
        const target = moveHorizontalPos(state, pos, -count);
        const targetBlock = textblockInfo(state, target);
        desiredCol = target - targetBlock.start;
        return target;
      }
      case "l":
      case "ArrowRight": {
      ...
      case "Ctrl-u":
        return moveVerticalPos(view, -1, count * LINE_JUMP_COUNT, pos);
      case "w": {
        const target = wordForwardPos(state, pos, count);
        const targetBlock = textblockInfo(state, target);
        desiredCol = target - targetBlock.start;
        return target;
      }
      case "b": {
        const target = wordBackwardPos(state, pos, count);
        const targetBlock = textblockInfo(state, target);
        desiredCol = target - targetBlock.start;
        return target;
      }
      case "e": {
        const target = wordEndPos(state, pos, count);
        const targetBlock = textblockInfo(state, target);
        desiredCol = target - targetBlock.start;
        return target;
      }
      ...
      case "^": {
        const target = firstNonBlankInBlock(state, pos);
        const targetBlock = textblockInfo(state, target);
        desiredCol = target - targetBlock.start;
        return target;
      }
  ...
  function deleteRange(state, dispatch, from, to, change, repeatAction) {
    ...
    const meta = withRepeat({
      register: text,
      registerType: isLinewise ? REGISTER_TYPES.LINE : REGISTER_TYPES.CHAR,
  ...
  function indentRange(state, dispatch, from, to, direction, times) {
    if (!dispatch) return true;
    const INDENT = "  ";
    const blocks = textblockList(state);
    ...
    var tr = state.tr;
    for (var i2 = lastIdx; i2 >= firstIdx; i2--) {
      var block = blocks[i2];
      if (direction === "right") {
        tr = tr.insertText(INDENT.repeat(times), block.start, block.start);
      } else {
        var text = tr.doc.textBetween(block.start, block.end);
        var charsToRemove = 0;
        var maxToRemove = INDENT.length * times;
        for (var c = 0; c < maxToRemove && c < text.length; c++) {
          if (text[c] === " " || text[c] === "\t") charsToRemove++;
          else break;
        }
        if (charsToRemove > 0) {
          tr = tr.delete(block.start, block.start + charsToRemove);
        }
      }
    }

    var firstBlock = blocks[firstIdx];
    var cursorPos = clamp(
      firstNonBlankInBlock({ doc: tr.doc, selection: state.selection }, firstBlock.start),
      0,
      tr.doc.content.size,
    );
    tr = tr.setSelection(textSelection(tr.doc, cursorPos, cursorPos));
    tr = setVimMeta(tr, withRepeat(clearTransient({
      mode: MODES.NORMAL,
      visualAnchor: null,
      visualLine: false,
    }), { type: "command", key: direction === "right" ? ">" : "<", count: times, lineCount: lastIdx - firstIdx + 1 }));
    dispatch(tr.scrollIntoView());
    return true;
  }
  ...
  function repeatLastChange(view, countOverride) {
    ...
    if (action.type === "command") {
      ...
      if (action.key === ">") {
        var r = currentLinewiseRange(view.state, action.lineCount || 1);
        return indentRange(view.state, view.dispatch, r.from, r.to, "right", count);
      }
      if (action.key === "<") {
        var l = currentLinewiseRange(view.state, action.lineCount || 1);
        return indentRange(view.state, view.dispatch, l.from, l.to, "left", count);
      }
    }
  ...
  function setMark(state, dispatch, markChar) {
    if (!dispatch) return true;
    var pluginState = getPluginState(state);
    var pos = normalCursorPos(state);
    var marks = {};
    if (pluginState.marks) {
      Object.keys(pluginState.marks).forEach(function (k) {
        marks[k] = pluginState.marks[k];
      });
    }
    marks[markChar] = pos;
    dispatch(
  ...
  function jumpToMark(state, dispatch, markChar) {
    var pluginState = getPluginState(state);
    var pos = (pluginState.marks || {})[markChar];
    if (pos == null) {
      dispatch(
        setVimMeta(state.tr, {
          pending: null,
          count: "",
          message: "mark " + markChar + " not set",
        }),
      );
      return true;
    }
    return setCursor(state, dispatch, pos, {
      pending: null,
      count: "",
      message: "jumped to mark " + markChar,
    });
  }

  function jumpToMarkLine(state, dispatch, markChar) {
    var pluginState = getPluginState(state);
    var pos = (pluginState.marks || {})[markChar];
    if (pos == null) {
      dispatch(
    ...
    var linePos = firstNonBlankInBlock(state, pos);
    return setCursor(state, dispatch, linePos, {
      pending: null,
      count: "",
      message: "jumped to mark " + markChar + " line",
    });
  }
  ...
  function normalCommand(view, key, options) {
    ...
    if (pluginState.pending === "m") {
      if (/^[a-z]$/.test(key)) return setMark(state, view.dispatch, key);
      return dispatchPending(state, view.dispatch, null, "mark must be a-z");
    }

    if (pluginState.pending === "`") {
      if (/^[a-z]$/.test(key)) return jumpToMark(state, view.dispatch, key);
      return dispatchPending(state, view.dispatch, null, "mark must be a-z");
    }

    if (pluginState.pending === "'") {
      if (/^[a-z]$/.test(key)) return jumpToMarkLine(state, view.dispatch, key);
      return dispatchPending(state, view.dispatch, null, "mark must be a-z");
    }
    ...
    if (key === "m") return dispatchPending(state, view.dispatch, "m", "m");
    if (key === "`") return dispatchPending(state, view.dispatch, "`", "`");
    if (key === "'") return dispatchPending(state, view.dispatch, "'", "'");
  ...
  function visualCommand(view, key) {
    ...
    if (pluginState.pending === "`") {
      if (/^[a-z]$/.test(key)) {
        var markPos = (pluginState.marks || {})[key];
        if (markPos == null) {
          return dispatchPending(state, view.dispatch, null, "mark " + key + " not set");
        }
        var range = visualSelectionRangeForCursor(state, pluginState.visualAnchor, markPos);
        return setRange(state, view.dispatch, range.anchor, range.head, {
          mode: MODES.VISUAL,
          visualAnchor: pluginState.visualAnchor,
          pending: null,
    ...
    if (pluginState.pending === "'") {
      if (/^[a-z]$/.test(key)) {
        var markPos2 = (pluginState.marks || {})[key];
        if (markPos2 == null) {
          return dispatchPending(state, view.dispatch, null, "mark " + key + " not set");
        }
        var markLinePos = firstNonBlankInBlock(state, markPos2);
        var range2 = visualSelectionRangeForCursor(state, pluginState.visualAnchor, markLinePos);
        return setRange(state, view.dispatch, range2.anchor, range2.head, {
          mode: MODES.VISUAL,
          visualAnchor: pluginState.visualAnchor,
          pending: null,
          count: "",
    ...
    if (key === "p") {
      ...
      const isLinePaste = pluginState.registerType === REGISTER_TYPES.LINE || register.indexOf("\n") !== -1;
      var tr = state.tr;
      var selectionPos;
      if (isLinePaste) {
        tr = tr.delete(state.selection.from, state.selection.to);
        var nodes = linewisePasteNodes(state, register, 1);
        var lineInsertAt = tr.mapping.map(state.selection.from);
        tr = tr.insert(lineInsertAt, nodes.length === 1 ? nodes[0] : nodes);
        selectionPos = clamp(lineInsertAt + 1, 0, tr.doc.content.size);
      } else {
        var insertAt = visualPasteInsertPos(state, pluginState);
        tr = tr.insertText(register, insertAt, insertAt);
        selectionPos = lastInsertedTextCursorPos(tr.doc, insertAt, register);
      }
      tr = tr.setSelection(textSelection(tr.doc, selectionPos, selectionPos));
      tr = setVimMeta(tr, {
        mode: MODES.NORMAL,
        ...
        state: {
          ...
          apply(tr, value) {
            ...
            const next = {
              ...
              marks: value.marks
                ? Object.fromEntries(
                    Object.entries(value.marks).map(function (entry) {
                      return [entry[0], tr.mapping.map(entry[1])];
                    }),
                  )
                : {},
            };
            ...
            if (Object.prototype.hasOwnProperty.call(meta, "marks")) {
              next.marks = meta.marks;
            }
