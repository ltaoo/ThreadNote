...
function createMiniEditor(host, options) {
  ...
  const removeIndent = installIndentShortcut(host, editor);
  const removeDrop = installFileDropHandler(host, editor);
  const removeAgentSelection = installMemoAgentSelection(host, {
    getSelection() {
      const view = editor.view;
      const selection = view && view.state && view.state.selection;
    ...
    replace(snapshot, text) {
      const view = snapshot.view;
      if (!view || !view.state || snapshot.to > view.state.doc.content.size) return false;
      if (view.state.doc.textBetween(snapshot.from, snapshot.to, "\n") !== snapshot.text) return false;
      const transaction = view.state.tr.insertText(text, snapshot.from, snapshot.to);
      const cursor = snapshot.from + text.length;
...
function installIndentShortcut(host, editor) {
  function onKeyDown(event) {
    if (event.defaultPrevented || event.key !== "Tab" || event.metaKey || event.ctrlKey || event.altKey) return;

    const view = editor && editor.view;
    if (!view || !view.state || !view.state.selection) return;
    ...
    if (!event.shiftKey && selection.empty) {
      view.dispatch(state.tr.insertText(MEMO_EDITOR_INDENT).scrollIntoView());
      return;
    }

    const lineStarts = selectedLineStarts(state.doc, selection);
    ...
    lineStarts
      ...
      .forEach(function (start) {
        if (!event.shiftKey) {
          transaction = transaction.insertText(MEMO_EDITOR_INDENT, start);
          return;
        }

        const prefix = state.doc.textBetween(start, Math.min(start + 2, state.doc.content.size), "", "");
        const removable = prefix.charAt(0) === "\t" ? "\t" : (prefix.match(/^ {1,2}/) || [""])[0];
        if (removable) transaction = transaction.delete(start, start + removable.length);
      });
...
function selectedLineStarts(doc, selection) {
  if (selection.empty) {
    return [selection.$from.start(selection.$from.depth)];
  }
  ...
  doc.forEach(function (node, offset) {
    ...
    if (end < selection.from || start > selection.to || start === selection.to) return;
    starts.push(start);
  });
  return starts;
}
...
function installMemoEditorPlugins(editor, options) {
  ...
  const plugins = [
    ...
    createMemoLocationSyntaxHighlightPlugin(editor),
    createMemoLocationPickerPlugin(editor),
  ];
...
function createMemoLocationSyntaxHighlightPlugin(editor) {
  const PM = window.ProsemirrorMod;
  const key = new PM.PluginKey(editor.id + "-memoLocationSyntaxHighlight");
  const locationPattern = /(^|[\s([{（【「『])(@@)([^\s<>()\[\]{}，。！？、；;,.]{1,64})/g;
    ...
    props: {
      decorations(state) {
        ...
        state.doc.descendants(function (node, pos) {
          ...
          let match = null;
          while ((match = locationPattern.exec(node.text))) {
            const from = pos + match.index + match[1].length;
            const to = from + match[2].length;
            decorations.push(
              PM.Decoration.inline(from, to, {
                class: "mini-location-token",
              }),
            );
          }
        });
...
function createMemoLocationPickerPlugin(editor) {
  const PM = window.ProsemirrorMod;
  const key = new PM.PluginKey(editor.id + "-memoLocationPicker");
  ...
  function findTrigger(state) {
    ...
    const prev = index > 0 ? before.charAt(index - 1) : "";
    if (prev && !/[\s([{（【「『]/.test(prev)) return null;

    const query = before.slice(index + trigger.length);
    if (!isActiveMemoLocationQuery(query)) return null;
  ...
  return new PM.Plugin({
    ...
    state: {
      ...
      apply(transaction, value, oldState, newState) {
        ...
        const items = memoLocationItems(trigger.query);
        let selectedIndex = value.selectedIndex || 0;
        if (meta && meta.type === "setSelectedIndex") selectedIndex = meta.selectedIndex || 0;
        selectedIndex = items.length ? Math.max(0, Math.min(selectedIndex, items.length - 1)) : 0;
    ...
    view(view) {
      return createFloatingMenuView({
        className: "location-picker-menu hidden",
        key,
        render(menu, pluginState) {
          renderLocationPickerMenu(menu, pluginState);
        },
        onMouseDown(event, pluginState) {
          const option = closestElement(event.target, "[data-location-picker-index]");
          if (!option) return false;
          event.preventDefault();
          const item = pluginState.items[Number(option.dataset.locationPickerIndex)];
          return selectItem(view, item);
        },
        view,
      });
...
function isActiveMemoLocationQuery(query) {
  const value = String(query || "");
  if (!value) return true;
  if (value.charAt(0) === "@") return false;
  if (/[\r\n]/.test(value)) return false;
...
function memoLocationItems(query) {
  const value = String(query || "").trim();
  if (value) {
    return [{ label: value, detail: "使用输入地点", value: value }];
  }
  return [];
}
...
function createFallbackEditor(host, options) {
  ...
  textarea.addEventListener("keydown", (event) => {
    if (event.key === "Tab" && !event.metaKey && !event.ctrlKey && !event.altKey) {
      event.preventDefault();
      updateTextareaIndent(textarea, event.shiftKey);
      if (options.onChange) options.onChange(textarea.value);
      return;
    }
    if ((event.metaKey || event.ctrlKey) && event.key === "Enter" && options.onSubmit) {
  ...
  const removeAgentSelection = installMemoAgentSelection(host, {
    ...
    replace(snapshot, text) {
      if (textarea.value.slice(snapshot.from, snapshot.to) !== snapshot.text) return false;
      textarea.value = textarea.value.slice(0, snapshot.from) + text + textarea.value.slice(snapshot.to);
      textarea.selectionStart = textarea.selectionEnd = snapshot.from + text.length;
      if (options.onChange) options.onChange(textarea.value);
      textarea.focus();
      return true;
    },
...
function installMemoAgentSelection(host, adapter) {
  ...
  function update() {
    window.setTimeout(function () {
      if (!host.isConnected || !host.contains(document.activeElement)) {
        toolbar.hidden = true;
        return;
      }
      const snapshot = adapter.getSelection();
      if (!snapshot || !snapshot.text) {
        toolbar.hidden = true;
        return;
      }
      toolbar.hidden = false;
      positionToolbar(toolbar, host, snapshot);
    }, 0);
  }
  ...
  function open(event) {
    ...
    openMemoAgentDialog({
      ...
      replace(text) {
        return adapter.replace(snapshot, text);
      },
    });
  }

  toolbar.addEventListener("mousedown", function (event) { event.preventDefault(); });
  toolbar.addEventListener("click", open);
  ...
  return function () {
    toolbar.removeEventListener("click", open);
...
function positionToolbar(toolbar, host, snapshot) {
  let left;
  let top;
  if (snapshot.view && typeof snapshot.view.coordsAtPos === "function") {
    try {
      const start = snapshot.view.coordsAtPos(snapshot.from);
      const end = snapshot.view.coordsAtPos(snapshot.to);
  ...
  toolbar.style.left = Math.max(28, Math.min(window.innerWidth - 28, left)) + "px";
  toolbar.style.top = Math.max(8, top) + "px";
}

function updateTextareaIndent(textarea, unindent) {
  const value = textarea.value;
  const selectionStart = textarea.selectionStart;
  const selectionEnd = textarea.selectionEnd;

  if (!unindent && selectionStart === selectionEnd) {
    textarea.value = value.slice(0, selectionStart) + MEMO_EDITOR_INDENT + value.slice(selectionEnd);
    textarea.selectionStart = textarea.selectionEnd = selectionStart + MEMO_EDITOR_INDENT.length;
    return;
  }

  const firstLineStart = value.lastIndexOf("\n", Math.max(0, selectionStart - 1)) + 1;
  const selectedEnd = selectionEnd > selectionStart && value.charAt(selectionEnd - 1) === "\n"
    ? selectionEnd - 1
    : selectionEnd;
  const lineStarts = [firstLineStart];
  let newline = value.indexOf("\n", firstLineStart);
  while (newline >= 0 && newline < selectedEnd) {
  ...
  lineStarts
    ...
    .forEach(function (start) {
      if (!unindent) {
        nextValue = nextValue.slice(0, start) + MEMO_EDITOR_INDENT + nextValue.slice(start);
        if (start <= selectionStart) nextStart += MEMO_EDITOR_INDENT.length;
        if (start < selectionEnd || selectionStart === selectionEnd) nextEnd += MEMO_EDITOR_INDENT.length;
        return;
      }

      const removable = nextValue.charAt(start) === "\t"
        ? "\t"
        : (nextValue.slice(start, start + 2).match(/^ {1,2}/) || [""])[0];
      if (!removable) return;
      nextValue = nextValue.slice(0, start) + nextValue.slice(start + removable.length);
      if (start < selectionStart) nextStart -= Math.min(removable.length, selectionStart - start);
      if (start < selectionEnd) nextEnd -= Math.min(removable.length, selectionEnd - start);
    });

  textarea.value = nextValue;
  textarea.selectionStart = Math.max(firstLineStart, nextStart);
  textarea.selectionEnd = Math.max(textarea.selectionStart, nextEnd);
}
