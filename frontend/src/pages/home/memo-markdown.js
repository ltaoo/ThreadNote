...
function inlineMarkdownBase(value, context = {}) {
  ...
  // Step 5b: Location syntax
  html = replaceMemoLocationSyntax(html);
...
function renderMemoLinkToken(label, url) {
  ...
  const displayUrl = href !== "#" ? href : url;
  const title = label || displayUrl;
  return `<a class="memo-inline-link" href="${escapeAttr(href)}" target="_blank" rel="noreferrer" title="${escapeAttr(title)}">${escapeHTML(label || displayUrl)}</a>`;
}
...
function replaceMemoLocationSyntax(html) {
  return String(html || "")
    .split(/(<code\b[^>]*>.*?<\/code>|<[^>]+>)/gi)
    .map(function (part) {
      if (!part || part.charAt(0) === "<") return part;
      return part.replace(
        /(^|[\s([{（【「『])(@@)([^\n<>()\[\]{}，。！？、；;,.]{1,64})/g,
        function (_, prefix, trigger, value) {
          return prefix + renderMemoLocationToken(trigger, value);
        },
      );
    })
    .join("");
...
function renderMemoLocationToken(trigger, value) {
