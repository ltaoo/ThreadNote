...
const task_tag_stage_backlog = "stage:backlog"
...
func createVaultTask(ctx *VaultContext, req TaskCreateRequest) (TaskRecord, error) {
	...
	task := TaskRecord{
		BoardID:       sanitizeBoardID(req.BoardID),
	...
	// Evaluate task.created rules before writing (may modify BoardID, Tags, Status, etc.)
	evaluateAndApplyTaskCreatedRules(ctx, &task)
	...
	fireTaskHooks(ctx, "task.created", task)
	return task, nil
}
...
func updateVaultTask(ctx *VaultContext, req TaskUpdateRequest) (TaskRecord, error) {
	...
	oldPath := path
	if req.BoardID != nil {
		task.BoardID = sanitizeBoardID(*req.BoardID)
	}
	if req.Title != nil {
		title := strings.TrimSpace(*req.Title)
		if title == "" {
	...
	_, _ = rebuildTaskIndex(ctx)
	if prevStatus != "" {
		if task.Status == taskStatusCompleted && prevStatus != taskStatusCompleted {
			fireTaskHooks(ctx, "task.completed", task)
		} else if task.Status == taskStatusOpen && (prevStatus == taskStatusCompleted || prevStatus == taskStatusCancelled) {
			fireTaskHooks(ctx, "task.reopened", task)
		}
	}
	return task, nil
}
...
func deleteVaultTask(ctx *VaultContext, id string) error {
	...
	_, _ = rebuildTaskIndex(ctx)
	fireTaskHooks(ctx, "task.deleted", task)
	return nil
}
...
func normalizeTaskStatus(value string) string {
	...
	case taskStatusOpen:
		return taskStatusOpen
	default:
		// Preserve the value as-is for board column IDs and other custom statuses.
...
func normalizeTaskRecord(task TaskRecord) TaskRecord {
	...
	task.BoardID = sanitizeBoardID(task.BoardID)
